...
# Add root folder to sys.path so we can import from scripts
...
@st.cache_data
def get_data():
    raw_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'raw', 'diabetes_hba1c_raw.csv'))
    try:
...
if page == "🏠 Home":
    ...
    st.markdown("Download the PDF report generated by standard EDA pipeline. Please run `main.py` first.")
    report_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'outputs', 'reports', 'EDA_Final_Report.pdf'))
